...
class WOLF extends EventEmitter {
  ...
  async createProxyAgent(config) {
    console.log("🚀 ~ WOLF ~ createProxyAgent ~ config:", config)
    try {
      ...
      console.log(`🔧 Creating proxy agent for: ${proxy.protocol}://${proxy.host}:${proxy.port}`);
      ...
      if (proxy.protocol === "socks5") {
        ...
        console.warn(`⚠️ Unsupported proxy protocol: ${proxy.protocol}`);
        return null;
      }
    } catch (error) {
  ...
  async login(config) {
    // console.log("🚀 ~ WOLF ~ login ~ config:", config)
    ...
    const socketOptions = {
      ...
      reconnectionDelay: 10,  // Start with 1s delay
      reconnectionDelayMax: 15,  // Cap at 5s (faster than 10s)
    ...
    if (config.proxy && config.proxy.enabled) {
      ...
      console.log("🚀 ~ WOLF ~ login ~ proxyAgent:", proxyAgent)
      if (proxyAgent) {
        socketOptions.agent = proxyAgent;
        console.log(`🌐 Using proxy: ${config.proxy.protocol}://${config.proxy.host}:${config.proxy.port}`);
      } else {
        console.error('⚠️ Proxy is enabled but failed to create proxy agent. Proceeding without proxy.');
        return null;
      }
    }
