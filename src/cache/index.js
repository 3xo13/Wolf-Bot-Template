...
class Cache {
  ...
  async getGame (id) {
    return JSON.parse(await this._get(`game.groupId:${id}`));
  }

  async setGame (id, value, ttl) {
    return await this._set(`game.groupId:${id}`, JSON.stringify(value), ttl);
  }

  async deleteGame (id) {
    return await this._delete(`game.groupId:${id}`);
  }
}
