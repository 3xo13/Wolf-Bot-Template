...
const getFormattingData = async (client, ads, links, opts) => {
  // Skip links entirely if flag is set (prevents mismatch when metadata fails)
  if (opts?.formatting?._skipLinks) {
    return undefined;
  }
...
const getEmbedData = async (client, formatting, options) => {
  ...
  // add thi to activate the embed fetching of links
  //  ...links ?? []
  for (const item of [...groupLinks ?? [],].flat().filter(Boolean)) {
    ...
    if (Reflect.has(item, 'url')) {
      ...
      try {
        // Add 5 second timeout to metadata fetch to prevent hanging
        const metadataPromise = client.misc.metadata(item.url);
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Metadata fetch timeout')), 5000)
        );

        const response = await Promise.race([metadataPromise, timeoutPromise]);

        if (!response.success) {
          // Mark to skip links on retry if metadata fails
          options.formatting._skipLinks = true;
          continue;
        }
        ...
        // Mark to skip links on retry if metadata fetch fails
        options.formatting._skipLinks = true;
        continue;
      }
    }
